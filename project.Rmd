...
library(scales)
library(tsbox)
airline_pass <- read.csv("salesweekly.csv", header = TRUE)
...
```{r}
airline_ts <- ts(airline_pass[,-1], start=2014, frequency=52)
...
airline_ts.train = window(airline_ts, end=train_end)
airline_ts.test = window(airline_ts, start=test_start)

ggtsdisplay(airline_ts.train, lag.max = 208, theme = theme_light(), main = "Original training data")

```
```{r}
ndiffs(airline_ts.train, alpha = 0.1, test = "adf")
```
```{r}
ndiffs(diff(diff(airline_ts.train, 52), 1), alpha = 0.05, test = "adf")
```
```{r}
nsdiffs(airline_ts.train)
```

```{r}
ggtsdisplay(diff(airline_ts.train,52), lag.max = 208, theme = theme_light(), main = "Stationary training data")
```

```{r}
M1.fit = sarima(airline_ts.train, p = 1, d = 0, q = 1, P = 1, D = 1, Q = 0, S = 52)
```

```{r}
library(forecast)
ndiffs(airline_ts.train) # Suggests value for `d`
nsdiffs(airline_ts.train) # Suggests value for `D`

```

```{r}
# new fit w/other package (forecast) to produce nice plots
M1.fit_plots = Arima(airline_ts.train, 
                     order = c(1, 0, 1), 
...
# forecasts plot
autoplot(ts_c(Observed = airline_ts, `Fixed training (12-step ahead forecast)` = M1.fct.v1$mean),
